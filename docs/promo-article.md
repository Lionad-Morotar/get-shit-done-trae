...
> **然而，仅靠压缩功能并不足够。** 开箱即用的情况下，即使像 Opus 4.5 这样的前沿编码模型，在 Claude Agent SDK 中以循环方式跨多个上下文窗口运行，若仅提供高层次的提示（例如"构建一个 claude.ai 的克隆网站"），也难以完成生产级 Web 应用的构建任务。
>
> **Claude 的失败主要表现为两种模式。** 首先，该代理倾向于一次性尝试完成过多任务——本质上是试图"一步到位"地实现整个应用。这常常导致模型在实现过程中耗尽上下文，使得下一次会话开始时功能仅被部分实现且缺乏文档说明。随后，代理不得不猜测之前发生了什么，并花费大量时间重新让基础应用恢复运行。即使采用了上下文压缩技术，它也并不总能将清晰明确的指令传递给下一个代理。
>
